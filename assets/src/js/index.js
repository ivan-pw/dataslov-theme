...
function shareButtonsInit(el) {
    ...
    for (let i = 0; i < social.length; i++) {
        const s = social[i];
        markup += '<a class="' + s.className + '" data-id="' + s.id + '">' + '<i class="' + s.icon + '"></i></a>';
    }
        ...
        fetch(url)
            ...
            .then((data) => {
                ...
                data.forEach((item) => {
                    ...
                    const articleType = document.querySelector(`.library-filter__type__item[data-term-id="${item.article_type}"]`)
                        .dataset.termName;
...
function clockStart() {
    ...
    clockWrapper.appendChild(clock).shadowRoot.querySelector('style').after(clockWrapper.querySelector('style'));
