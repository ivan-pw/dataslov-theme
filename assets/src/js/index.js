...
// import { ShareButtons, shareBtnContainer } from 'share-buttons';
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  const circleContainer = document.querySelector(
    'section.words-slider .circle-container'
  );

  if (circleContainer) {
    fetch(
      domain + '/wp-json/wp/v2/word_year?filter[orderby]=created&order=desc'
    )
      .then((response) => {
      ...
      .then((data) => {
        ...
        data.forEach((el) => {
          if (+el.count > 0 && +el.words_count > 0) {
            itemsNav += `
  ...
  if (document.querySelector('article .filter')) {
    filtersInit();
    // document
    //   .querySelector(`article .filter .filter__year .filter__item[data-term-name="${new Date().getFullYear()}"]`)
    //   .click();
  }
      ...
      .then((data) => {
        ...
        data.forEach((item) => {
          ...
          const articleType = document.querySelector(
            `.library-filter__type__item[data-term-id="${item.article_type}"]`
          ).dataset.termName;
...
function clockStart() {
  ...
  clockWrapper
    ...
    .after(clockWrapper.querySelector('style'));
...
function startSlider() {
  ...
  circleContainer.querySelectorAll('li').forEach((el, i, arr) => {
    ...
    el.style.transform = `rotate(${lastRotate}deg) translate(${
      circleContainer.offsetHeight / 3
    }px)`; // 200
  ...
  function onSliderInit(yearEl) {
    ...
    getWordsList(yearEl.dataset.yearId)
      .then((data) => {
        ...
        wordList.innerHTML =
          `<h4>Другие слова ${yearEl.dataset.year} года: </h4>` + items;
